...
const uploadImage = async()=>{
  ...
  uploadTask.on(
    ...
    (error) => {
      setImageFileUploadError("Could not upload image(File must be less than 4MB)");
...
const handleDeleteUser = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      toast.warn('User account has been deleted');
    }
  } catch (error) {
  ...
  return (
    <div className='max-w-lg mx-auto p-3 w-full'>
      <h1 className="my-7 text-center font-extrabold text-4xl text-white-700 ">Profile</h1>
      <form onSubmit={handleSubmit} className='flex flex-col gap-6'>
        ...
        <TextInput type='text' id='email'placeholder='EMAIL' defaultValue={currentUser.email} onChange={handleChange} ></TextInput>
